...
uint DevComm_Send(devcomm_int t_Device,
    ...
    tp_Link->tp_SendSegmentHead = (devcomm_segment_head *) (tp_Link->u8_SendBuffer +
...
uint DevComm_WriteDeviceDone(devcomm_int t_Device) {
    ...
    if (tp_Link->t_SendPacketCount > 0) {
        tp_Link->tp_SendPacketHead = (devcomm_packet_head *) ((uint8 *) tp_PacketHead +
...
static uint DevComm_CheckPayloadLength(devcomm_link *tp_Link,
    ...
    devcomm_int t_PacketCount =
            (sizeof(devcomm_segment_head) + t_Length + t_PacketPayload - 1) / t_PacketPayload;
...
static uint DevComm_ReceivePacket(devcomm_int t_Device,
    ...
    if (tp_PacketHead->u8_Checksum != tp_Callback->fp_GetCRC8((const uint8 *) tp_PacketHead,
                                                              sizeof(devcomm_packet_head) -
                                                              sizeof(tp_PacketHead->u8_Checksum),
                                                              CHECKSUM_BASE_CRC8))
        return DevComm_DiscardPacket(t_Device, tp_Control, t_Length);
    ...
    if (tp_PacketHead->t_Offset == 0) {
        ...
        if (REG_READ_FIELD(tp_ReceivePacketHead->u8_Control, PACKET_CONTROL_OFFSET_IDENTITY,
                           PACKET_IDENTITY_MASK) !=
            REG_READ_FIELD(tp_PacketHead->u8_Control, PACKET_CONTROL_OFFSET_IDENTITY,
                           PACKET_IDENTITY_MASK))
            return DevComm_DiscardPacket(t_Device, tp_Control, t_Length);

        //Check if length of packet is out of range
        if (tp_PacketHead->t_Length >
            (tp_Link->u8_ReceiveBuffer + DEVCOMM_LINK_BUFFER_SIZE) -
            ((uint8 *) tp_ReceivePacketHead + tp_ReceivePacketHead->t_Length))
            return DevComm_DiscardPacket(t_Device, tp_Control, t_Length);
...
static uint DevComm_ParsePacket(devcomm_int t_Device,
    ...
    u16_Checksum = tp_Callback->fp_GetCRC16(u8p_Buffer,
                                            ...
                                            sizeof(tp_SegmentHead->u16_Checksum),
                                            CHECKSUM_BASE_CRC16);
    ...
    if (REG_GET_BIT(tp_Link->t_Flag, DEVCOMM_LINK_FLAG_ACKNOWLEDGEMENT) != 0) {
        for (u16_RetryCounter = 0;
             u16_RetryCounter <= tp_Link->u16_RetryCounter; u16_RetryCounter++) {
    ...
    if (t_PayloadLength > 0) {
        ...
        if ((REG_READ_FIELD(tp_PacketHead->u8_Control, PACKET_CONTROL_OFFSET_IDENTITY,
                            PACKET_IDENTITY_MASK) != tp_Link->u8_ReceiveIdentity) ||
            (REG_READ_FIELD(tp_PacketHead->u8_Control, PACKET_CONTROL_OFFSET_IDENTITY,
                            PACKET_IDENTITY_MASK) == 0)) {
            tp_Link->u8_ReceiveIdentity = REG_READ_FIELD(tp_PacketHead->u8_Control,
                                                         PACKET_CONTROL_OFFSET_IDENTITY,
                                                         PACKET_IDENTITY_MASK);
...
static void DevComm_ReloadSendBuffer(devcomm_link *tp_Link,
    ...
    tp_PacketHead->t_Length =
            sizeof(devcomm_packet_head) + sizeof(devcomm_segment_head) + t_DataLength;
...
static void DevComm_Retry(devcomm_int t_Device,
    ...
    if (tp_Link->tp_ReceiveSegmentHead != 0)
        u16_Checksum = tp_Link->tp_ReceiveSegmentHead->u16_Checksum;
    else
        u16_Checksum = 0;
